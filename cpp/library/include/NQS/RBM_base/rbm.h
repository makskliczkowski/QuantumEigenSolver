...
{
	/**
	* @brief Restricted Boltzmann Machine (RBM) class template.
	* 
	* @tparam _spinModes Number of spin modes.
	* @tparam _Ht Hamiltonian type.
	* @tparam _T Data type (default is the same as Hamiltonian type).
	* @tparam _stateType State type (default is double).
	*/
	...
	public:														// -------------------- P R O B A B I L I T Y --------------------
		...
		virtual auto logPRatio(uint, uint, float, float)		-> _T override = 0;
