...
_T RBM_S<2, _Ht, _T, _stateType>::logPRatio(uint fP, float fV)
{
	...
	val				=	val * this->bV_(fP) + RBM_ACCU_ARMA(arma::log(arma::cosh(this->theta_ + val * this->W_.col(fP))) - this->thetaCOSH_log_);
#else
...
#	ifdef NQS_NOT_OMP_MT
	...
	val				=	val + RBM_ACCU_ARMA(arma::log(arma::cosh(thetaTMP)) - this->thetaCOSH_log_);
#	else
	// val				=	std::exp(val) * arma::prod(arma::cosh(this->thetaTmp_) / this->thetaCOSH_);
	val				=	val + RBM_ACCU_ARMA(arma::log(arma::cosh(this->thetaTmp_)) - this->thetaCOSH_log_);
#	endif
#else
	val				=   val * this->bV_(fP) + RBM_ACCU_ARMA(arma::log(this->coshF(this->tmpVec_)) - arma::log(this->coshF(this->curVec_)));
#endif
	return val;
}
...
#ifdef NQS_ANGLES_UPD
	val		+= RBM_ACCU_ARMA(arma::log(this->coshF(_v1)) - this->thetaCOSH_log_);
#else
	// val		+= arma::sum(arma::log(this->coshF(_v1) / this->coshF(this->curVec_)));
	val		+= RBM_ACCU_ARMA(arma::log(this->coshF(_v1)) - arma::log(this->coshF(this->curVec_)));
#endif
	return val;
}
