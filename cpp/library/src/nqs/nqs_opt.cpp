                                                    ...
                                                    Timer& _timer)
{
	...
	this->blockSample(_par.MC_th_, NQS_STATE, false);			// thermalize the system - burn-in
                                                        ...
                                                        Timer& _timer)
{
    try
    {
        ...
        if (_collectEn)                                             // save the mean energy and the standard deviation
            MonteCarlo::blockmean(*_E, _par.nblck_, &(*_EM)(i - 1), &(*_ES)(i - 1)); 
...
{									
	...
	arma::Col<_T> meanEn;
	{
		...
		if (_collectEn) 
        {
            ...
            if (_energiesStd)
                _energiesStd->resize(_par.MC_sam_ * _par.nblck_);
		}
	}
    Timer _timer;
